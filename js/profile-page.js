...
async function loadUserStatistics() {
	try {
		...
		document.getElementById(
			"global-ranking"
		).textContent = `#${rankings.global.rank}`;
		document.getElementById(
			"africa-ranking"
		).textContent = `#${rankings.africa.rank}`;
		document.getElementById(
			"europe-ranking"
		).textContent = `#${rankings.europe.rank}`;
		document.getElementById(
			"asia-ranking"
		).textContent = `#${rankings.asia.rank}`;
		document.getElementById(
			"america-ranking"
		).textContent = `#${rankings.america.rank}`;
...
async function renderCharts(stats) {
	try {
		...
		} = await import("./charts-config.js");

		// Render games over time chart
		if (stats.gamesOverTime.dates.length > 0) {
			const ctx1 = document
				.getElementById("games-over-time-chart")
				.getContext("2d");
		...
		if (difficultyKeys.length > 0) {
			const avgMoves = difficultyKeys.map(
				(diff) => stats.performanceByDifficulty[diff].avgMoves
			);
			const avgTimes = difficultyKeys.map(
				(diff) => stats.performanceByDifficulty[diff].avgTime
			);

			const ctx2 = document
				.getElementById("performance-by-difficulty-chart")
				.getContext("2d");
		...
		if (continentKeys.length > 0) {
			const avgMoves = continentKeys.map(
				(cont) => stats.performanceByContinent[cont].avgMoves
			);
			const avgTimes = continentKeys.map(
				(cont) => stats.performanceByContinent[cont].avgTime
			);

			const ctx3 = document
				.getElementById("performance-by-continent-chart")
				.getContext("2d");
